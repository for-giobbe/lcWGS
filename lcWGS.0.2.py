...
with open('log_mitoz', 'w') as mitoz_log:
        subprocess.call(["mitoz","annotate","--fastafiles","mtcontig.fa","--outprefix","annot","--thread_number",args.threads,"--species_name",args.output], stdout=mitoz_log, stderr=mitoz_log)

print("#    UCE annotation  " + datetime.datetime.now().strftime("%H:%M:%S")) #####################################################################################################################################################

with open('log_phyluce', 'w') as phyluce_log:
	subprocess.call(["phyluce_assembly_match_contigs_to_probes",'--min_coverage',80,'--min_identity',80,"--contigs","folder_spades/contigs.fasta","--probes",database_uc], stdout=phyluce_log, stderr=phyluce_log)
	#subprocess.call(["phyluce_assembly_get_match_counts",'--min_coverage',80,'--min_identity',80,"--contigs","folder_spades/contigs.fasta","--probes",database_uc]
